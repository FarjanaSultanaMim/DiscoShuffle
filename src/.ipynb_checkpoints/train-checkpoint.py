...
def main(args):
    ...
    essayids, essays, _, scores, prompts, _ = data.load_annotated_essay_with_normalized_score('Path_to_essay_file(.xlsx)', score_source="data/{}Scores.txt".format(args.mp_score_type))  # ./data/ICLE_essays.xlsx
    ...
    if args.mp_di_aware:
        ...
        essays = data.preprocess_essay_withParaBoundary(essays, args, boseos=True, bopeop=True)
    elif args.mp_model_type == "nea_aft_pretrain" and not args.mp_para:
        essays = data.preprocess_essay_noParaBoundary(essays, args, boseos=True)
    ...
    if args.mp_prompt:
        prompts = data.preprocess_essay_noParaBoundary(essays, args, boseos=True)
